...
| [skill-evals.md](skill-evals.md) | Three ways a skill-triggering measurement lies: one run is a sample not a result (3 of 19 cases flip), measuring a skill alone overstates it ~3× versus its installed group, and "did it fire first" undercounts because agents explore before deciding. Read before quoting an eval number, or before rewording a description to make one go green — a 2×2 experiment measured that rewording does nothing for the skills that never fire. Also, from three measured skills: value is headroom times capture, and both must be measured — a +2 can mean 'already good at this' or 'failing to land', which need opposite responses. |
